...
function command(script, args, detached, dataCb, processIdCb) {
    if (!dataCb) {
    ...
    const cmd = spawn('/usr/bin/python3 ' + scriptPath + script, args, {shell: true, detached:detached});
    cmd.unref();
...
function imageClassification(params, cb) {
    ...
    command('image-classifier.py', args, false, function(err, data) {
        if ( err ) {
           cb(err)
        }
        else {
...
function objectDetection(params, cb) {
    ...
    var outputFileSent = false;
    command('object-detector.py', args, true, function(err, data) {
        if ( err ) {
           cb(err)
        }
        else {
